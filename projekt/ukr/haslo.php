...
<body>
    ...
    if (isset($_POST['jezyk'])) {
        header("location: ../haslo.php?kod=$kod");
        exit;
    }
    ?>
    <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function validatePassword() {
                    ...
                    if (password.length < 8) {
                        passwordInput.setCustomValidity('Пароль має містити не менше 8 символів.');
                        return;
                    }

                    // Sprawdzenie, czy hasło zawiera co najmniej jedną dużą literę
                    if (!/[A-Z]/.test(password)) {
                        passwordInput.setCustomValidity('Пароль має містити принаймні одну велику літеру.');
                        return;
                    }

                    // Sprawdzenie, czy hasło zawiera co najmniej jedną cyfrę
                    if (!/\d/.test(password)) {
                        passwordInput.setCustomValidity('Пароль має містити хоча б одну цифру.');
                        return;
                    }

                    // Sprawdzenie, czy hasło zawiera co najmniej jeden znak specjalny
                    if (!/[@$!%*?&]/.test(password)) {
                        passwordInput.setCustomValidity('Пароль повинен містити хоча б один спеціальний символ.');
                        return;
                    }
